...
export interface GuildTickResult {
  guildId: string;
  action: "noop" | "expired" | "spawned" | "skipped_no_channel" | "empty_pool";
  dailyWaifuId?: string;
  characterId?: string;
}
...
async function getGuildCycleHours(guildId: string): Promise<number> {
  const rows = await db
    .select({ cycleDurationHours: guilds.cycleDurationHours })
    .from(guilds)
    .where(eq(guilds.id, guildId))
    .limit(1);
  return rows[0]?.cycleDurationHours ?? 24;
}

export async function tickOnceForGuild(guildId: string): Promise<GuildTickResult> {
  ...
  const cycleHours = await getGuildCycleHours(guildId);
  const expiresAt = new Date(Date.now() + cycleHours * 60 * 60_000);

  const dailyWaifuId = await insertSpawn(guildId, choice, expiresAt);
  logger.info(
    { guildId, dailyWaifuId, characterId: choice.characterId, characterName: choice.name },
    "Daily waifu spawned",
  );

  return { guildId, action: "spawned", dailyWaifuId, characterId: choice.characterId };
}

export async function tickOnceAll(): Promise<GuildTickResult[]> {
  ...
  for (const g of configuredGuilds) {
    const result = await tickOnceForGuild(g.id);
    if (result.action === "skipped_no_channel") continue;
    results.push(result);
  }
